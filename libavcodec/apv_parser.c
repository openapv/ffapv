...
static int parse_apv_bitstream(AVCodecParserContext *s, AVCodecContext *avctx, const uint8_t *buf, int buf_size)
{
    ...
    while (data_size > 0) {
        ...
        if (data_size < APV_FRAME_DATA_SIZE_PREFIX_LENGTH)
            return AVERROR_INVALIDDATA;

        frame_data_size = apv_read_frame_data_size(data, data_size, avctx);


        data += APV_FRAME_DATA_SIZE_PREFIX_LENGTH;
        data_size -= APV_FRAME_DATA_SIZE_PREFIX_LENGTH;

        if (data_size < frame_data_size)
            return AVERROR_INVALIDDATA;

        ret = parse_frame_data(s, avctx, data, frame_data_size);
        if (ret < 0) {
            av_log(avctx, AV_LOG_ERROR, "Parsing of Frame Data failed\n");
            return AVERROR_INVALIDDATA;
        }

        data += frame_data_size;
